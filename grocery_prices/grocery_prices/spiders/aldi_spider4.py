...
class Aldi4Spider(scrapy.Spider):
    name = 'aldi4'

    def __init__(self, *args, **kwargs):
        super(Aldi4Spider, self).__init__(*args, **kwargs)
        self.driver = webdriver.Chrome(service=Service(ChromeDriverManager().install()))

    def start_requests(self):
        # First request to the specific link
        initial_url = 'https://new.aldi.us/products/fresh-produce/salad-kits/k/167'
        yield scrapy.Request(url=initial_url, callback=self.parse, meta={'page': 1, 'total_pages': 2, 'initial': True})
    ...
    def parse(self, response):
        ...
        if initial or next_page <= total_pages:
            ...
            base_url = 'https://new.aldi.us/products/fresh-produce/salad-kits/k/167?page={page}'
